...
public struct GoXLRDynamicValue<Parent: ObservableObject & GoXLRCommandConvertible, Value: Equatable, Selection: Hashable>: DynamicProperty {
    ...
    private let paths: [Selection:KeyPath<Parent, Value>]
    @Binding var selection: Selection
    
    @ObservedObject private var updater = Updater()
    private var cancellable: AnyCancellable? = nil
    
    public init(_ parentKeyPath: KeyPath<Mixer, Parent>, _ paths: [Selection:KeyPath<Parent, Value>], selection: Binding<Selection>) {
        ...
        for path in paths.values {
            cancellable = GoXlr.shared.mixer![keyPath: parentKeyPath].publisher(for: path).sink { [self] newValue in
                //Maybe check if value has really been updated and not just re-set ?
                guard path == valueKeyPath else { return }
